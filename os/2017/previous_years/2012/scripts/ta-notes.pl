...
sub usage {
	print "Usage: ta-notes [lab1|threads|userprog|vm|filesys] <user-id>\n";
	exit(1);
}
...
if (!(-e $notes_file)) {
	...
	print $notes_fp "=============================\n";
	print $notes_fp "ta-notes for $lab_id for '@group_names', submission made at $submission_timestr.\n";
...
if (xsystem("/usr/bin/vi + $tmpfile") != 0) {
	print "vi exited with error. Notes NOT taken.\n";
	exit(1);
}
