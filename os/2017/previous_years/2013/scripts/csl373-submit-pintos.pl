...
sub usage {
	print "Usage: csl373-submit-pintos [base|userprog|vm|filesys].\n";
	exit(1);
}
...
foreach my $lid ("base", "userprog", "vm", "filesys") {
	if ($lab_id eq $lid) {
		$lab_id_found = 1;
...
print "Enter the URL of your repository (e.g., https://svn.iitd.ernet.in/~cs1112345/pintos-repo):\n";
my $svn_url = <STDIN>;
...
foreach my $partner (@partners) {
	print "csl373-get-submission $partner $lab_id\n";
}
exit(0);
...
sub organize_submission
{
	...
	my $diff_file = "$subdir/diff.src.$prev_assn.$prev_id";
	my $exitstatus = xsystem("/usr/bin/diff -urpN $prevdir/src  $subdir/src > $diff_file");
...
sub process_files_userprog
{
	...
	print "Enter the submission-ID of your base submission (e.g., base/grp.cs1112345.cs5067890.0.2011-1-26.12:12:12). If you have forgotten your submission ID, you can retrieve it by typing 'csl373-get-submission $user base' which will create a directory named by your base submission ID. You will need to prefix it with 'base/':\n";
	...
	if (organize_submission($subdir, "userprog", $base_submission_id, "base")) {
		return 1;
	}
	return 0;
}
...
sub process_files_vm
{
	...
	print "Enter the submission-ID of your userprog submission (e.g., userprog/grp.cs1112345.cs5067890.0.2011-2-26.12:12:12). If you have forgotten your submission ID, you can retrieve it by typing 'csl373-get-submission $user userprog' which will create a directory named by your userprog submission ID. You will need to prefix it with 'userprog/'.\n";
...
sub process_files_filesys
{
	...
	print "Enter the submission-ID of your vm submission (e.g., vm/grp.cs1112345.cs5067890.0.2011-2-26.12:12:12 or userprog/grp.cs1112345.cs5067890.0.2012-2-26.12:12:12). If you have forgotten your submission ID, you can retrieve it by typing 'csl373-get-submission $user vm' or 'csl373-get-submission $user userprog' which will create a directory named by your vm/userprog submission ID. You will need to prefix it with 'vm/' or 'userprog/' depending on the case.\n";
	my $submission_id = <STDIN>;
	chomp($submission_id);
	my $prev_submission_id;
	my $prev_subdir = "$SUBMISSION_DIRECTORY/$submission_id";
	my $prev_assid;
	if ($submission_id =~ /^vm\/(.*)$/) {
		$prev_assid = 'vm';
		$prev_submission_id = $1;
	} elsif ($submission_id =~ /^userprog\/(.*)$/) {
		$prev_assid = 'userprog';
		$prev_submission_id = $1;
	} else {
		print "You should prefix your submission ID with 'vm/' or 'userprog/'\n";
		return 1;
	}
	if (!(-e $prev_subdir)) {
		print "Your $prev_assid submission ID is invalid: '$prev_submission_id'.\n";
		return 1;
	}
	$prev_submission_id = untaint($prev_submission_id);
	if (organize_submission($subdir, "filesys", $prev_submission_id, $prev_assid)) {
		return 1;
	}
	return 0;
}
